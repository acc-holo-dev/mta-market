...
Область: PostgreSQL 16, **Prisma 8 contract-ORM** (schema = `apps/server/src/prisma/contract.prisma`,
конфиг `apps/server/prisma.config.ts`, клиент `@prisma/orm-postgres/runtime`),
production/staging.

Важно: это НЕ классический Prisma с `schema.prisma`/`prisma migrate`.
В Prisma 8 два пути применения схемы (см. `apps/server/.cursor/skills/prisma-8/references/migrations.md`):
...
В PLAN-004 строка `apps/server/migrations/` убрана из `.gitignore`; новые
